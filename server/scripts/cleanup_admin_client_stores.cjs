#!/usr/bin/env node
/*
  CommonJS cleanup_admin_client_stores.cjs
  ...
  Usage examples:
    node server/scripts/cleanup_admin_client_stores.cjs         # dry-run (no changes)
    node server/scripts/cleanup_admin_client_stores.cjs --delete   # archive then delete
    node server/scripts/cleanup_admin_client_stores.cjs --backup-only # just backup
