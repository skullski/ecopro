...
export default function FurnitureTemplate(props: TemplateProps) {
  ...
  // Theme colors - Neutral/warm furniture theme
  ...
  const accent = asString(settings.template_accent_color) || '#78716c';
  const headerBg = asString(settings.template_header_bg) || '#ffffff';
  const headerText = asString(settings.template_header_text) || '#1c1917';
  const cardBg = asString(settings.template_card_bg) || '#ffffff';
  const footerBg = asString(settings.template_footer_bg) || cardBg;
  ...
  const heroKicker = asString(settings.template_hero_kicker) || 'FURNITURE · INTERIOR · DESIGN';
  const ctaText = asString(settings.template_button_text) || 'EXPLORE COLLECTION';
  const ctaSecondaryText = asString(settings.template_button_secondary_text) || 'DESIGN SERVICES';
  const heroBadge = asString(settings.template_hero_badge) || '🏠 NEW COLLECTION';
  const heroImage = asString(settings.banner_url) || (props.products?.[0]?.images?.[0] || '/placeholder.png');

  const storeName = asString(settings.store_name) || 'HABITAT';
  ...
  const sectionTitle = asString(settings.template_featured_title) || 'New Arrivals';
  const sectionSubtitle = asString(settings.template_featured_subtitle) || 'Fresh designs for your home';
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '64px' }}>
            ...
            >
              ...
              <span style={{ color: headerText, fontWeight: headingWeight as any, fontSize: '18px', letterSpacing: '0.05em' }}>
                {storeName}
              </span>
            </div>
        ...
        <main style={{ flex: 1 }}>
          ...
          >
            <div style={{ flex: 1 }}>
              ...
              <p
                style={{ color: muted, fontSize: '15px', lineHeight: 1.7, marginBottom: '28px', maxWidth: '420px' }}
          ...
          {/* Products Grid */}
          <section
            style={{ padding: isMobile ? '24px 16px' : '32px' }}
      ...
      >
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'center' : 'flex-start', gap: '32px', marginBottom: '32px', textAlign: isMobile ? 'center' : 'left' }}>
            <div>
              <div style={{ color: text, fontWeight: 500, fontSize: '18px', marginBottom: '8px' }}>{storeName}</div>
              <div
          ...
          {socialLinks.length > 0 && (
            <div
              style={{ display: 'flex', gap: '20px', justifyContent: isMobile ? 'center' : 'flex-start', paddingTop: '24px', borderTop: '1px solid #e7e5e4' }}
