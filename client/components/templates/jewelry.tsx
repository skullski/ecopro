...
import { navigateToProduct, navigateToCheckout } from '@/utils/checkout';
...
export default function JewelryTemplate(props: TemplateProps) {
  ...
  // Helper functions to save product data and navigate (database-backed, not localStorage)
  const handleProductClick = (product: any) => {
    navigateToProduct(product.id, product, navigate, storeSlug).catch(console.error);
  };

  const handleBuyClick = (product: any, e?: any) => {
    if (e) e.stopPropagation();
    navigateToCheckout(product.id, product, navigate, storeSlug).catch(console.error);
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center max-w-md mx-auto p-4 md:p-6">
          ...
          <h1 className="text-xl md:text-2xl font-serif font-bold mb-4 text-gray-900">No Products Yet</h1>
  ...
  const collections = ['All', ...new Set(products.map((p: any) => p.collection || 'Uncategorized'))];
  const filtered = activeCollection === 'All' ? products : products.filter((p: any) => (p.collection === activeCollection));
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="border-b border-gray-200 bg-white sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 py-6">
          <h1 className="font-serif text-2xl md:text-xl md:text-2xl font-light tracking-wider text-gray-900">{storeName}</h1>
          <p className="text-xs text-gray-600 mt-2">Luxury jewelry for every occasion</p>
        </div>
      </header>

      {/* Featured Section */}
      <section className="border-b border-gray-200 bg-gradient-to-br from-gray-50 to-white py-6 md:py-4 md:py-6">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="font-serif text-xl md:text-2xl font-semibold mb-4 text-gray-900">Featured Collection</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {products.slice(0, 4).map((p: any) => (
              <div key={p.id} className="bg-white border border-gray-200 rounded-lg p-4 text-center hover:shadow-md transition group cursor-pointer" onClick={() => setQuickViewProduct(p)}>
                <img src={p.images?.[0]} alt={p.title} className="w-full h-40 object-cover rounded mb-3 group-hover:scale-105 transition" />
                <p className="text-sm font-medium text-gray-900">{p.title}</p>
                <p className="text-gray-600 text-xs mt-1">{p.collection || 'Premium'}</p>
            ...
            {filtered.map((p: any) => (
              ...
              >
                <div className="relative h-48 bg-gray-100 overflow-hidden">
                  <img src={p.images?.[0]} alt={p.title} className="w-full h-full object-cover group-hover:scale-110 transition" />
                </div>
                <div className="p-4 flex flex-col flex-grow">
                  <p className="text-sm font-medium text-gray-900">{p.title}</p>
                  <p className="text-xs text-gray-600 mt-1">{p.collection || 'Premium'}</p>
      ...
      {quickViewProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur">
          <div className="bg-white rounded-lg w-full max-w-md p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              <p className="font-semibold text-lg text-gray-900">{quickViewProduct.title}</p>
              <p className="text-sm text-gray-600 mt-1">{quickViewProduct.collection || 'Premium'}</p>
      ...
      {/* Footer */}
      <footer className="border-t border-gray-200 mt-6 md:mt-4 md:mt-6 py-4 md:py-6 bg-gray-50">
