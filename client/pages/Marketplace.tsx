...
import { Search, Tag, User, Heart, Star, ShoppingCart, MessageCircle, Phone, BadgeCheck } from "lucide-react";
...
export default function Marketplace() {
  ...
  return (
    <section className="container mx-auto py-4 px-1 md:px-4 dark:bg-[#18191b] min-h-screen">
      {/* Category Navigation */}
      <div className="flex gap-2 overflow-x-auto scrollbar-hide mb-4 py-2 px-1">
        <button
          className={cn("px-4 py-1.5 rounded-full font-semibold border border-primary bg-primary text-white hover:bg-primary/80 transition text-xs", !category && "ring-2 ring-primary")}
          onClick={() => setCategory("")}
        >
          All
        </button>
        {categories.map(cat => (
          <button
            key={cat}
            className={cn("px-4 py-1.5 rounded-full font-semibold border border-gray-700 bg-[#232325] text-gray-100 hover:bg-primary/10 transition text-xs", category === cat && "ring-2 ring-primary border-primary bg-primary/10 text-primary")}
