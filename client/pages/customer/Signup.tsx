...
export default function CustomerSignup(){
  ...
  function handleSignup(e:any){
    ...
    if(exists){ alert('الحساب موجود'); return; }
  ...
  return (
    <section className="container mx-auto py-20">
      <div className="mx-auto max-w-md rounded-lg border bg-card p-6">
        <h3 className="text-lg font-bold mb-4">إنشاء حساب زبون</h3>
        <form onSubmit={handleSignup} className="grid gap-3">
          <input value={name} onChange={(e)=>setName(e.target.value)} placeholder="الاسم" className="rounded-md border bg-background px-3 py-2" />
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" onClick={()=>navigate(`/s/${id}`)}>العودة للمتجر</Button>
          </div>
        </form>
      </div>
    </section>
