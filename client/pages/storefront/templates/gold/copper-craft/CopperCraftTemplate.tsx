...
export default function CopperCraftTemplate(props: TemplateProps) {
  ...
  const storeName = asString(settings.store_name) || 'COPPER CRAFT';
  ...
  const bg = asString(settings.template_bg_color) || '#0b0b0b';
  const text = asString(settings.template_text_color) || '#fef3c7';
  const muted = asString(settings.template_muted_color) || '#d1d5db';
  ...
  const heroTitle = asString(settings.template_hero_heading) || 'Crafted. Strong. Timeless.';
  const heroSubtitle = asString(settings.template_hero_subtitle) || 'Warm metallic highlights with dark contrast.';
  const heroKicker = asString(settings.template_hero_kicker) || 'HANDMADE • WORKSHOP';
  const ctaText = asString(settings.template_button_text) || 'Shop';
  const bannerUrl = asString(settings.banner_url);

  const featuredTitle = asString(settings.template_featured_title) || 'Featured work';
  const featuredSubtitle = asString(settings.template_featured_subtitle) || 'Built to last. Designed to impress.';
  ...
  return (
    <div className="ecopro-storefront" data-edit-path="__root" style={{ minHeight: '100vh', background: bg, color: text, fontFamily }}>
      <header data-edit-path="layout.header" onClick={(e) => clickGuard(e, 'layout.header')} style={{ borderBottom: '1px solid rgba(245,158,11,0.22)', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', position: 'sticky', top: 0, zIndex: 20 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: `${spacing}px ${spacing}px`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14 }}>
          <div data-edit-path="layout.header.logo" onClick={(e) => clickGuard(e, 'layout.header.logo')} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: canManage ? 'pointer' : 'default' }}>
            {logoUrl && <img src={logoUrl} alt={storeName} style={{ width: 38, height: 38, borderRadius: 10, objectFit: 'cover', border: `1px solid rgba(245,158,11,0.35)` }} />}
          ...
          <nav data-edit-path="layout.header.nav" onClick={(e) => clickGuard(e, 'layout.header.nav')} style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            {navLinks.map((l) => (
              <button key={`${l.label}-${l.url}`} onClick={(e) => { if (canManage) return clickGuard(e, 'layout.header.nav'); navigate(l.url || '/products'); }} style={{ background: 'transparent', border: '1px solid rgba(245,158,11,0.18)', color: text, borderRadius: borderRadius, padding: '7px 10px', fontSize: 12, fontWeight: 900, cursor: 'pointer' }}>
                {l.label || 'Link'}
              </button>
            ))}
          </nav>
      ...
      <section data-edit-path="layout.hero" onClick={(e) => clickGuard(e, 'layout.hero')} style={{ padding: `${sectionSpacing}px ${spacing}px`, position: 'relative' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(900px 420px at 15% 20%, rgba(245,158,11,0.18), transparent 60%), radial-gradient(800px 360px at 80% 30%, rgba(244,63,94,0.10), transparent 60%)' }} />
        {bannerUrl && (
          <div data-edit-path="layout.hero.image" onClick={(e) => clickGuard(e, 'layout.hero.image')} style={{ position: 'absolute', inset: 0, opacity: 0.20 }}>
        ...
        <div style={{ position: 'relative', maxWidth: 1200, margin: '0 auto', display: 'grid', gap: 16 }}>
          ...
          <p data-edit-path="layout.hero.subtitle" onClick={(e) => clickGuard(e, 'layout.hero.subtitle')} style={{ margin: 0, maxWidth: 720, color: 'rgba(254,243,199,0.82)', fontSize: 15, lineHeight: 1.8 }}>{heroSubtitle}</p>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 6 }}>
            <button data-edit-path="layout.hero.cta" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.hero.cta'); navigate('/products'); }} style={{ background: accent, color: '#0b0b0b', border: 0, borderRadius: buttonRadius, padding: '12px 18px', fontWeight: 950, cursor: 'pointer' }}>{ctaText}</button>
            <button data-edit-path="layout.hero.cta2" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.hero.cta2'); navigate('/products'); }} style={{ background: 'transparent', color: text, border: '1px solid rgba(245,158,11,0.22)', borderRadius: buttonRadius, padding: '12px 18px', fontWeight: 950, cursor: 'pointer' }}>{asString(settings.template_button2_text) || 'See details'}</button>
          </div>
        </div>
      </section>
      ...
      <section data-edit-path="layout.featured" onClick={(e) => clickGuard(e, 'layout.featured')} style={{ padding: `0 ${spacing}px ${sectionSpacing}px` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 16, flexWrap: 'wrap', marginBottom: 16 }}>
            ...
            <div style={{ color: muted, fontSize: 12 }}>Copper accents</div>
          </div>

          <div data-edit-path="layout.products">
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${gridColumns}, minmax(0, 1fr))`, gap: gridGap }}>
              {products.map((p) => (
                <div key={p.id} data-edit-path={`layout.products.${p.id}`} onClick={(e) => { if (canManage) return clickGuard(e, `layout.products.${p.id}`); if ((p as any).slug) navigate((p as any).slug); }} style={{ background: cardBg, borderRadius: cardRadius, overflow: 'hidden', border: '1px solid rgba(245,158,11,0.18)', cursor: 'pointer' }}>
      ...
      <footer data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')} style={{ padding: `${spacing}px ${spacing}px`, borderTop: '1px solid rgba(245,158,11,0.22)', background: 'rgba(0,0,0,0.35)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div data-edit-path="layout.footer.social" onClick={(e) => clickGuard(e, 'layout.footer.social')} style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {(socialLinks.length ? socialLinks : canManage ? [{ platform: 'instagram', url: 'https://instagram.com' }] : []).map((s) => (
              <a key={`${s.platform}-${s.url}`} href={canManage ? undefined : s.url} onClick={(e) => { if (canManage) return clickGuard(e as any, 'layout.footer.social'); e.stopPropagation(); }} style={{ color: text, textDecoration: 'none', fontSize: 12, padding: '6px 10px', borderRadius: borderRadius, border: '1px solid rgba(245,158,11,0.18)' }}>
