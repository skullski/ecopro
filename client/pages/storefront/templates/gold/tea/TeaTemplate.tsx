...
export default function TeaTemplate(props: TemplateProps) {
  ...
  const heroTitle = settings.template_hero_heading || 'Find Your Calm 🍵';
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: bg, color: text, fontFamily: 'Georgia, serif' }} data-edit-path="__root">
      ...
      <section style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }} data-edit-path="layout.products">
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: '24px' }}>
          {products.map(product => (
            <div key={product.id} style={{ backgroundColor: cardBg, borderRadius: '16px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', cursor: canManage ? 'default' : 'pointer' }} data-edit-path={`layout.products.${product.id}`}
      ...
      <footer style={{ padding: '48px 24px', backgroundColor: '#ecfccb', marginTop: '48px', textAlign: 'center' }} data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')}>
        <p style={{ color: '#4d7c0f', fontStyle: 'italic' }}>© {new Date().getFullYear()} {storeName} 🍃</p>
      </footer>
    </div>
  );
}
