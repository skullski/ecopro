...
export default function TerraMarketTemplate(props: TemplateProps) {
  ...
  const storeName = asString(settings.store_name) || 'TERRA MARKET';
  ...
  const bg = asString(settings.template_bg_color) || '#fbf7f0';
  ...
  const heroTitle = asString(settings.template_hero_heading) || 'Warm finds, daily essentials.';
  const heroSubtitle = asString(settings.template_hero_subtitle) || 'Earthy design with clear hierarchy and cozy cards.';
  const heroKicker = asString(settings.template_hero_kicker) || 'FRESH • LOCAL • HANDPICKED';
  const ctaText = asString(settings.template_button_text) || 'Browse';
  const bannerUrl = asString(settings.banner_url);

  const featuredTitle = asString(settings.template_featured_title) || 'Best Sellers';
  const featuredSubtitle = asString(settings.template_featured_subtitle) || 'Quality items people come back for.';
  ...
  const buttonRadius = resolveInt(settings.template_button_border_radius, 14, 0, 50);
  ...
  return (
    <div className="ecopro-storefront" data-edit-path="__root" style={{ minHeight: '100vh', background: bg, color: text, fontFamily }}>
      <header data-edit-path="layout.header" onClick={(e) => clickGuard(e, 'layout.header')} style={{ borderBottom: '1px solid rgba(31,41,55,0.08)', background: 'rgba(251,247,240,0.85)', backdropFilter: 'blur(6px)', position: 'sticky', top: 0, zIndex: 20 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: `${spacing}px ${spacing}px`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          <div data-edit-path="layout.header.logo" onClick={(e) => clickGuard(e, 'layout.header.logo')} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: canManage ? 'pointer' : 'default' }}>
            {logoUrl && <img src={logoUrl} alt={storeName} style={{ width: 40, height: 40, borderRadius: 9999, objectFit: 'cover', border: `2px solid ${accent}` }} />}
      ...
      <section data-edit-path="layout.hero" onClick={(e) => clickGuard(e, 'layout.hero')} style={{ padding: `${sectionSpacing}px ${spacing}px` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 22, alignItems: 'center' }}>
          <div>
            ...
            <h1 data-edit-path="layout.hero.title" onClick={(e) => clickGuard(e, 'layout.hero.title')} style={{ margin: '12px 0 0', fontSize: 44, lineHeight: 1.08, fontWeight: 950 }}>{heroTitle}</h1>
            <p data-edit-path="layout.hero.subtitle" onClick={(e) => clickGuard(e, 'layout.hero.subtitle')} style={{ margin: '14px 0 0', color: muted, fontSize: 15, lineHeight: 1.7, maxWidth: 560 }}>{heroSubtitle}</p>
            <div style={{ display: 'flex', gap: 12, marginTop: 18, flexWrap: 'wrap' }}>
              <button data-edit-path="layout.hero.cta" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.hero.cta'); navigate('/products'); }} style={{ background: accent, color: '#fff', border: 0, borderRadius: buttonRadius, padding: '12px 16px', fontWeight: 900, cursor: 'pointer' }}>{ctaText}</button>
              <button data-edit-path="layout.hero.cta2" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.hero.cta2'); navigate('/products'); }} style={{ background: 'transparent', color: text, border: `1px solid rgba(31,41,55,0.18)`, borderRadius: buttonRadius, padding: '12px 16px', fontWeight: 900, cursor: 'pointer' }}>{asString(settings.template_button2_text) || 'Learn More'}</button>
            </div>
          </div>

          <div style={{ position: 'relative', borderRadius: cardRadius, overflow: 'hidden', border: '1px solid rgba(31,41,55,0.10)', background: '#f3ede3' }}>
            <div style={{ aspectRatio: '4/3', width: '100%' }}>
              {bannerUrl ? (
                <div data-edit-path="layout.hero.image" onClick={(e) => clickGuard(e, 'layout.hero.image')} style={{ width: '100%', height: '100%' }}>
                  <img src={bannerUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
              ) : (
                <div data-edit-path="layout.hero.image" onClick={(e) => clickGuard(e, 'layout.hero.image')} style={{ width: '100%', height: '100%', display: 'grid', placeItems: 'center', color: muted, fontWeight: 800 }}>
                  Upload a hero image
                </div>
              )}
            </div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div data-edit-path="layout.products">
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${gridColumns}, minmax(0, 1fr))`, gap: gridGap }}>
              {products.map((p) => (
                <div key={p.id} data-edit-path={`layout.products.${p.id}`} onClick={(e) => { if (canManage) return clickGuard(e, `layout.products.${p.id}`); if ((p as any).slug) navigate((p as any).slug); }} style={{ background: cardBg, borderRadius: cardRadius, overflow: 'hidden', border: '1px solid rgba(31,41,55,0.08)', cursor: 'pointer' }}>
                  ...
                  <div style={{ padding: 12, display: 'grid', gap: 8 }}>
                    ...
                    <button data-edit-path="layout.featured.addLabel" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.featured.addLabel'); e.stopPropagation(); if ((p as any).slug) navigate((p as any).slug); }} style={{ justifySelf: 'start', background: '#111827', color: '#fff', border: 0, borderRadius: borderRadius, padding: '8px 10px', fontSize: 12, fontWeight: 900, cursor: 'pointer' }}>
                      {addLabel}
                    </button>
                  </div>
                </div>
      ...
      <footer data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')} style={{ padding: `${spacing}px ${spacing}px`, borderTop: '1px solid rgba(31,41,55,0.08)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div data-edit-path="layout.footer.social" onClick={(e) => clickGuard(e, 'layout.footer.social')} style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {(socialLinks.length ? socialLinks : canManage ? [{ platform: 'instagram', url: 'https://instagram.com' }] : []).map((s) => (
              <a key={`${s.platform}-${s.url}`} href={canManage ? undefined : s.url} onClick={(e) => { if (canManage) return clickGuard(e as any, 'layout.footer.social'); e.stopPropagation(); }} style={{ color: text, textDecoration: 'none', fontSize: 12, padding: '6px 10px', borderRadius: borderRadius, border: '1px solid rgba(31,41,55,0.12)' }}>
