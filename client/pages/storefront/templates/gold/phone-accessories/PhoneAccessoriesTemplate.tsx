  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: bg, color: text, fontFamily: 'system-ui, sans-serif' }} data-edit-path="__root">
      ...
      <section style={{ padding: `${sectionSpacing}px ${baseSpacing}px`, maxWidth: '1400px', margin: '0 auto' }} data-edit-path="layout.grid">
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: `${gridGap}px` }}>
          {products.map(product => (
            <div key={product.id} style={{ backgroundColor: cardBg, borderRadius: `${cardRadius}px`, overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,0.06)', cursor: canManage ? 'default' : 'pointer', transition: `transform ${animationSpeed}ms, box-shadow ${animationSpeed}ms` }} data-edit-path={`layout.grid.items.${product.id}`}
              ...
              <div style={{ padding: '16px' }}>
                <p style={{ fontSize: '10px', color: productPriceColor, fontWeight: 600, marginBottom: '4px' }}>{product.category || 'ACCESSORY'}</p>
      ...
      <footer style={{ padding: `${sectionSpacing}px ${baseSpacing}px`, backgroundColor: '#fff', marginTop: `${sectionSpacing}px`, textAlign: 'center', borderTop: '1px solid #e4e4e7' }} data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')}>
        <p style={{ color: '#71717a' }}>{copyright}{storeDescription ? ` — ${storeDescription}` : ''}</p>
      </footer>
    </div>
  );
}
