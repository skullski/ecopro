...
export default function OceanSplashTemplate(props: TemplateProps) {
  ...
  const storeName = asString(settings.store_name) || 'OCEAN SPLASH';
  ...
  const bg = asString(settings.template_bg_color) || '#061826';
  const text = asString(settings.template_text_color) || '#e6f6ff';
  const muted = asString(settings.template_muted_color) || '#93c5fd';
  const accent = asString(settings.template_accent_color) || '#22d3ee';

  const fontFamily = asString(settings.template_font_family) || 'system-ui, -apple-system, Segoe UI, sans-serif';

  const featuredTitle = asString(settings.template_featured_title) || 'Featured Drops';
  const featuredSubtitle = asString(settings.template_featured_subtitle) || 'Fresh picks, ready to ship.';

  const heroKicker = asString(settings.template_hero_kicker) || 'NEW SEASON • COASTAL ENERGY';
  const heroTitle = asString(settings.template_hero_heading) || 'Dive into the New Collection';
  const heroSubtitle = asString(settings.template_hero_subtitle) || 'Clean layouts, bold colors, smooth shopping.';
  ...
  return (
    <div
      ...
      style={{ minHeight: '100vh', background: bg, color: text, fontFamily }}
    >
      <header
      ...
      >
        <div
          style={{
            maxWidth: 1200,
            margin: '0 auto',
            padding: `${Math.max(10, spacing - 2)}px ${spacing}px`,
            display: 'flex',
            alignItems: 'center',
        ...
        >
          ...
          >
            {logoUrl && (
              <img
                src={logoUrl}
                alt={storeName}
                style={{ width: 38, height: 38, borderRadius: 9999, objectFit: 'cover', border: `2px solid ${accent}` }}
              />
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <div style={{ fontWeight: 900, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{storeName}</div>
              {(canManage || storeDescription) && (
                <div style={{ fontSize: 12, color: muted, lineHeight: 1.2 }}>
          ...
          <nav
            ...
            style={{ display: 'flex', gap: 18, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end' }}
          >
            {navLinks.map((l) => (
              <button
                key={`${l.label}-${l.url}`}
      ...
      >
        <div
          style={{
            ...
            inset: 0,
            background:
              'radial-gradient(1200px 500px at 10% 20%, rgba(34,211,238,0.25), transparent 60%), radial-gradient(900px 420px at 80% 20%, rgba(59,130,246,0.22), transparent 60%), radial-gradient(800px 500px at 40% 90%, rgba(244,63,94,0.12), transparent 60%)',
          }}
        />
        {bannerUrl && (
          <div
        ...
        <div style={{ position: 'relative', maxWidth: 1200, margin: '0 auto', display: 'grid', gap: 22 }}>
          <div
            data-edit-path="layout.hero.kicker"
            onClick={(e) => clickGuard(e, 'layout.hero.kicker')}
            style={{ color: muted, fontSize: 12, letterSpacing: '0.22em', textTransform: 'uppercase' }}
          >
            {heroKicker}
          </div>
          <h1
            ...
            style={{ margin: 0, fontSize: 48, lineHeight: 1.05, fontWeight: 950 }}
          >
            {heroTitle}
          </h1>
          <p
            ...
            style={{ margin: 0, maxWidth: 680, color: 'rgba(230,246,255,0.85)', fontSize: 16, lineHeight: 1.7 }}
          >
            {heroSubtitle}
          </p>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            ...
            <button
              data-edit-path="layout.hero.cta2"
              onClick={(e) => {
                if (canManage) return clickGuard(e, 'layout.hero.cta2');
                navigate('/products');
              }}
              style={{
                background: 'transparent',
            ...
            >
              {asString(settings.template_button2_text) || 'Explore'}
            </button>
          </div>
        </div>
      </section>
      ...
      <section
        ...
        style={{ padding: `0 ${spacing}px ${sectionSpacing}px` }}
      >
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 18 }}>
            <div
              ...
              style={{ fontSize: 20, fontWeight: 900, letterSpacing: '0.08em', textTransform: 'uppercase' }}
            >
              {featuredTitle}
            </div>
            <div
          ...
          <div data-edit-path="layout.products" style={{ padding: 0 }}>
            <div
              style={{
                display: 'grid',
            ...
            >
              {products.map((p) => (
                <div
                  key={p.id}
                  data-edit-path={`layout.products.${p.id}`}
                  onClick={(e) => {
                    if (canManage) return clickGuard(e, `layout.products.${p.id}`);
                ...
                >
                  ...
                  <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ color: titleColor, fontWeight: 800, lineHeight: 1.2 }}>{productTitle(p)}</div>
      ...
      <footer
        ...
        style={{
          padding: `${Math.max(26, spacing)}px ${spacing}px`,
      ...
      >
        ...
        >
          <div data-edit-path="layout.footer.copyright" onClick={(e) => clickGuard(e, 'layout.footer.copyright')} style={{ color: muted, fontSize: 12 }}>
            {copyright}
          </div>
