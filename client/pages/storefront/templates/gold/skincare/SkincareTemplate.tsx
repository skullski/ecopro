...
export default function SkincareTemplate(props: TemplateProps) {
  ...
  const bg = settings.template_bg_color || '#faf5ff';
  const text = settings.template_text_color || '#4c1d95';
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: bg, color: text, fontFamily: 'system-ui, sans-serif' }} data-edit-path="__root">
      ...
      <section style={{ padding: '80px 24px', textAlign: 'center', background: 'linear-gradient(180deg, #f3e8ff 0%, #faf5ff 100%)' }} data-edit-path="layout.hero" onClick={(e) => clickGuard(e, 'layout.hero')}>
        <span style={{ display: 'inline-block', backgroundColor: accent, color: '#fff', padding: '6px 16px', borderRadius: '50px', fontSize: '11px', fontWeight: 600, marginBottom: '20px' }}>DERMATOLOGIST APPROVED</span>
        <h2 style={{ fontSize: breakpoint === 'mobile' ? '32px' : '52px', fontWeight: 700, marginBottom: '16px' }} data-edit-path="layout.hero.title" onClick={(e) => clickGuard(e, 'layout.hero.title')}>{heroTitle}</h2>
        <p style={{ color: '#7c3aed', fontSize: '18px', marginBottom: '32px' }} data-edit-path="layout.hero.subtitle" onClick={(e) => clickGuard(e, 'layout.hero.subtitle')}>{heroSubtitle}</p>
      ...
      <section style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }} data-edit-path="layout.products">
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: '24px' }}>
          {products.map(product => (
            <div key={product.id} style={{ backgroundColor: cardBg, borderRadius: '24px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(139,92,246,0.1)', cursor: canManage ? 'default' : 'pointer' }} data-edit-path={`layout.products.${product.id}`}
      ...
      <footer style={{ padding: '48px 24px', backgroundColor: '#f3e8ff', marginTop: '48px', textAlign: 'center' }} data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')}>
