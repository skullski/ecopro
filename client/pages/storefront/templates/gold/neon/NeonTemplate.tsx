...
export default function NeonTemplate(props: TemplateProps) {
  ...
  const accent = asString(settings.template_accent_color) || '#f0abfc';
  const cardBg = asString(settings.template_card_bg) || '#171717';
  ...
  const storeName = asString(settings.store_name) || 'NEON NIGHTS';
...
const heroTitle = asString(settings.template_hero_heading) || 'Glow Different 💫';
  const heroSubtitle = asString(settings.template_hero_subtitle) || 'Stand out from the crowd';
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: bg, color: text, fontFamily: 'system-ui, sans-serif' }} data-edit-path="__root">
      <header style={{ padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} data-edit-path="layout.header" onClick={(e) => clickGuard(e, 'layout.header')}>
        <h1 style={{ fontSize: '24px', fontWeight: 900, color: accent, textShadow: `0 0 20px ${accent}80` }} data-edit-path="layout.header.logo" onClick={(e) => clickGuard(e, 'layout.header.logo')}>{settings.store_logo ? (<img src={settings.store_logo} alt={storeName} style={{ width: 40, height: 40, borderRadius: '9999px', objectFit: 'cover' }} />) : null}⚡ {storeName}</h1>
      </header>
      ...
      <section style={{ padding: `${sectionSpacing}px ${baseSpacing}px`, maxWidth: '1400px', margin: '0 auto' }} data-edit-path="layout.grid">
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: gridGap }}>
          {products.map(product => (
            <div key={product.id} style={{ backgroundColor: cardBg, overflow: 'hidden', border: `1px solid ${accent}33`, borderRadius: cardRadius, cursor: canManage ? 'default' : 'pointer', transition: `transform ${animationSpeed}ms, box-shadow ${animationSpeed}ms` }} data-edit-path={`layout.grid.items.${product.id}`}
              onMouseEnter={(e) => { e.currentTarget.style.transform = `scale(${hoverScale})`; e.currentTarget.style.boxShadow = `0 0 20px ${accent}50`; }}
              ...
              <div style={{ padding: '16px' }}>
                <p style={{ fontSize: '10px', color: productPriceColor, fontWeight: 700, marginBottom: '4px', textShadow: `0 0 10px ${accent}` }}>{product.category || 'NEON'}</p>
      ...
      <footer style={{ padding: `${sectionSpacing}px ${baseSpacing}px`, marginTop: `${sectionSpacing}px`, textAlign: 'center', borderTop: `1px solid ${accent}33` }} data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')}>
