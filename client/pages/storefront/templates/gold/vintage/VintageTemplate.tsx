  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: bg, color: text, fontFamily: '"Playfair Display", Georgia, serif' }} data-edit-path="__root">
      <header style={{ padding: '20px 24px', display: 'flex', justifyContent: 'center', alignItems: 'center', borderBottom: `2px solid ${accent}` }} data-edit-path="layout.header" onClick={(e) => clickGuard(e, 'layout.header')}>
        <h1 style={{ fontSize: '28px', fontWeight: 600, fontStyle: 'italic' }} data-edit-path="layout.header.logo" onClick={(e) => clickGuard(e, 'layout.header.logo')}>{settings.store_logo ? (<img src={settings.store_logo} alt={storeName} style={{ width: 40, height: 40, borderRadius: '9999px', objectFit: 'cover' }} />) : null}{storeName}</h1>
      </header>
      ...
      <section style={{ padding: '80px 24px', textAlign: 'center' }} data-edit-path="layout.hero" onClick={(e) => clickGuard(e, 'layout.hero')}>
        <div style={{ maxWidth: '700px', margin: '0 auto', border: `3px double ${accent}`, padding: '60px 40px' }}>
          <p style={{ fontSize: '14px', letterSpacing: '6px', color: productPriceColor, marginBottom: '20px' }}>EST. 1952</p>
      ...
      <section style={{ padding: `${sectionSpacing}px ${baseSpacing}px`, maxWidth: '1400px', margin: '0 auto' }} data-edit-path="layout.grid">
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: `${gridGap}px` }}>
          {products.map(product => (
            <div key={product.id} style={{ backgroundColor: cardBg, borderRadius: `${cardRadius}px`, overflow: 'hidden', boxShadow: '0 2px 15px rgba(0,0,0,0.08)', cursor: canManage ? 'default' : 'pointer', transition: `transform ${animationSpeed}ms ease` }} data-edit-path={`layout.grid.items.${product.id}`}
              ...
              <div style={{ padding: '20px', textAlign: 'center' }}>
                <p style={{ fontSize: '10px', letterSpacing: '2px', color: productPriceColor, marginBottom: '8px' }}>{product.category || 'ANTIQUE'}</p>
                <h3 style={{ fontSize: '16px', fontWeight: 600, fontStyle: 'italic', marginBottom: '10px', color: productTitleColor}}>{product.title}</h3>
      ...
      <footer style={{ padding: `${sectionSpacing}px ${baseSpacing}px`, borderTop: `2px solid ${accent}`, marginTop: `${sectionSpacing}px`, textAlign: 'center' }} data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')}>
        <p style={{ fontStyle: 'italic', color: '#78716c' }}>{copyright}{storeDescription ? ` — ${storeDescription}` : ''}</p>
      </footer>
    </div>
  );
}
