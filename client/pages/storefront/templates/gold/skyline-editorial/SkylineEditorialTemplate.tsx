...
export default function SkylineEditorialTemplate(props: TemplateProps) {
  ...
  const storeName = asString(settings.store_name) || 'SKYLINE EDITORIAL';
  ...
  const fontFamily = asString(settings.template_font_family) || 'ui-serif, Georgia, Cambria, Times New Roman, Times, serif';

  const spacing = resolveInt(settings.template_spacing, 18, 8, 32);
  const sectionSpacing = resolveInt(settings.template_section_spacing, 72, 24, 120);

  const heroTitle = asString(settings.template_hero_heading) || 'A clean editorial storefront.';
  const heroSubtitle = asString(settings.template_hero_subtitle) || 'Large type, crisp layout, and calm product grid.';
  const heroKicker = asString(settings.template_hero_kicker) || 'FEATURE • WEEKLY';
  const ctaText = asString(settings.template_button_text) || 'Read & Shop';
  const bannerUrl = asString(settings.banner_url);

  const featuredTitle = asString(settings.template_featured_title) || 'Selected';
  const featuredSubtitle = asString(settings.template_featured_subtitle) || 'A short list of standouts.';
  ...
  return (
    <div className="ecopro-storefront" data-edit-path="__root" style={{ minHeight: '100vh', background: bg, color: text, fontFamily }}>
      <header data-edit-path="layout.header" onClick={(e) => clickGuard(e, 'layout.header')} style={{ borderBottom: '1px solid rgba(17,24,39,0.10)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: `${spacing}px ${spacing}px`, display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 14 }}>
          <div data-edit-path="layout.header.logo" onClick={(e) => clickGuard(e, 'layout.header.logo')} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: canManage ? 'pointer' : 'default' }}>
            {logoUrl && <img src={logoUrl} alt={storeName} style={{ width: 36, height: 36, borderRadius: 10, objectFit: 'cover', border: `1px solid rgba(17,24,39,0.16)` }} />}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <div style={{ fontWeight: 900, letterSpacing: '0.04em', textTransform: 'uppercase' }}>{storeName}</div>
              {(canManage || storeDescription) && <div style={{ fontSize: 12, color: muted }}>{storeDescription || (canManage ? 'Add store description...' : '')}</div>}
            </div>
          </div>

          <nav data-edit-path="layout.header.nav" onClick={(e) => clickGuard(e, 'layout.header.nav')} style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
            {navLinks.map((l) => (
              <button key={`${l.label}-${l.url}`} onClick={(e) => { if (canManage) return clickGuard(e, 'layout.header.nav'); navigate(l.url || '/products'); }} style={{ background: 'transparent', border: 0, color: text, fontSize: 13, cursor: 'pointer', padding: '6px 6px', borderBottom: `2px solid transparent` }}>
                {l.label || 'Link'}
              </button>
            ))}
          </nav>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <button data-edit-path="layout.header.cta" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.header.cta'); navigate('/products'); }} style={{ background: accent, color: '#fff', border: 0, borderRadius: buttonRadius, padding: '10px 14px', fontWeight: 900, cursor: 'pointer' }}>
              {asString(settings.template_header_cta_text) || 'Shop'}
            </button>
          </div>
        </div>
      </header>
      ...
      <section data-edit-path="layout.hero" onClick={(e) => clickGuard(e, 'layout.hero')} style={{ padding: `${sectionSpacing}px ${spacing}px` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gap: 18 }}>
          ...
          <h1 data-edit-path="layout.hero.title" onClick={(e) => clickGuard(e, 'layout.hero.title')} style={{ margin: 0, fontSize: 58, lineHeight: 1.02, fontWeight: 900 }}>
            {heroTitle}
          </h1>
          <p data-edit-path="layout.hero.subtitle" onClick={(e) => clickGuard(e, 'layout.hero.subtitle')} style={{ margin: 0, maxWidth: 760, fontSize: 16, color: muted, lineHeight: 1.8 }}>
            {heroSubtitle}
          </p>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <button data-edit-path="layout.hero.cta" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.hero.cta'); navigate('/products'); }} style={{ background: '#111827', color: '#fff', border: 0, borderRadius: buttonRadius, padding: '12px 16px', fontWeight: 900, cursor: 'pointer' }}>{ctaText}</button>
            <button data-edit-path="layout.hero.cta2" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.hero.cta2'); navigate('/products'); }} style={{ background: 'transparent', color: text, border: '1px solid rgba(17,24,39,0.16)', borderRadius: buttonRadius, padding: '12px 16px', fontWeight: 900, cursor: 'pointer' }}>{asString(settings.template_button2_text) || 'View collection'}</button>
          </div>

          {bannerUrl && (
            <div data-edit-path="layout.hero.image" onClick={(e) => clickGuard(e, 'layout.hero.image')} style={{ marginTop: 18, borderRadius: cardRadius, overflow: 'hidden', border: '1px solid rgba(17,24,39,0.10)' }}>
      ...
      <section data-edit-path="layout.featured" onClick={(e) => clickGuard(e, 'layout.featured')} style={{ padding: `0 ${spacing}px ${sectionSpacing}px` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 16, flexWrap: 'wrap', marginBottom: 14 }}>
            <div>
              <div data-edit-path="layout.featured.title" onClick={(e) => clickGuard(e, 'layout.featured.title')} style={{ fontWeight: 950, fontSize: 18 }}>{featuredTitle}</div>
            ...
            <div style={{ fontSize: 12, color: muted }}>Curated grid</div>
          </div>

          <div data-edit-path="layout.products">
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${gridColumns}, minmax(0, 1fr))`, gap: gridGap }}>
              {products.map((p) => (
                <div key={p.id} data-edit-path={`layout.products.${p.id}`} onClick={(e) => { if (canManage) return clickGuard(e, `layout.products.${p.id}`); if ((p as any).slug) navigate((p as any).slug); }} style={{ background: cardBg, borderRadius: cardRadius, overflow: 'hidden', border: '1px solid rgba(17,24,39,0.10)', cursor: 'pointer' }}>
                  <div style={{ aspectRatio: '4/3', overflow: 'hidden', background: '#f3f4f6' }}>
                    <img src={productImage(p)} alt={productTitle(p)} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </div>
                  <div style={{ padding: 12, display: 'grid', gap: 8 }}>
                    <div style={{ color: titleColor, fontWeight: 900, lineHeight: 1.25 }}>{productTitle(p)}</div>
                    <div style={{ color: priceColor, fontWeight: 900 }}>{formatPrice(Number(p.price) || 0)}</div>
                    <button data-edit-path="layout.featured.addLabel" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.featured.addLabel'); e.stopPropagation(); if ((p as any).slug) navigate((p as any).slug); }} style={{ justifySelf: 'start', background: 'rgba(37,99,235,0.12)', color: text, border: '1px solid rgba(37,99,235,0.20)', borderRadius: borderRadius, padding: '8px 10px', fontSize: 12, fontWeight: 900, cursor: 'pointer' }}>
                      {addLabel}
                    </button>
                  </div>
                </div>
      ...
      <footer data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')} style={{ padding: `${spacing}px ${spacing}px`, borderTop: '1px solid rgba(17,24,39,0.10)' }}>
