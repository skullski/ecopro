...
export default function CandlesTemplate(props: TemplateProps) {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: bg, color: text, fontFamily: 'Georgia, serif' }} data-edit-path="__root">
      ...
      <section style={{ padding: '80px 24px', textAlign: 'center', background: `radial-gradient(circle at center, ${accent}22 0%, transparent 50%)` }} data-edit-path="layout.hero" onClick={(e) => clickGuard(e, 'layout.hero')}>
        <p style={{ fontSize: '12px', letterSpacing: '4px', color: accent, marginBottom: '16px' }}>HANDCRAFTED WITH LOVE</p>
        ...
        <button style={{ backgroundColor: accent, color: bg, padding: '14px 40px', fontSize: '14px', fontWeight: 600, border: 'none', borderRadius: '4px', cursor: 'pointer' }} data-edit-path="layout.hero.cta" onClick={(e) => clickGuard(e, 'layout.hero.cta')}>{ctaText}</button>
      </section>

      {categories.length > 0 && (
        <div style={{ padding: '24px', display: 'flex', gap: '16px', flexWrap: 'wrap', justifyContent: 'center' }}>
          {['All', ...categories].map(cat => (
            <button key={cat} onClick={() => setCategoryFilter(cat === 'All' ? '' : cat)} style={{ padding: '10px 24px', backgroundColor: (cat === 'All' ? !categoryFilter : categoryFilter === cat) ? accent : 'transparent', color: (cat === 'All' ? !categoryFilter : categoryFilter === cat) ? bg : accent, border: `1px solid ${accent}`, fontSize: '13px', fontStyle: 'italic', cursor: 'pointer' }}>{cat}</button>
          ))}
        </div>
      )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: `${gridGap}px` }}>
          {products.map(product => (
            <div key={product.id} style={{ backgroundColor: cardBg, borderRadius: cardRadius, overflow: 'hidden', cursor: canManage ? 'default' : 'pointer', transition: `transform ${animationSpeed}ms` }} data-edit-path={`layout.products.${product.id}`}
              ...
              <div style={{ aspectRatio: '1', backgroundColor: '#44403c', overflow: 'hidden' }}>
