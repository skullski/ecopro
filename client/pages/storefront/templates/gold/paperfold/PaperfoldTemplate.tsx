...
export default function PaperfoldTemplate(props: TemplateProps) {
  ...
  const storeName = asString(settings.store_name) || 'PAPERFOLD';
  ...
  const accent = asString(settings.template_accent_color) || '#0ea5e9';
  ...
  const heroTitle = asString(settings.template_hero_heading) || 'Simple. Folded. Elegant.';
  const heroSubtitle = asString(settings.template_hero_subtitle) || 'Crisp paper-like panels and clean product grid.';
  const heroKicker = asString(settings.template_hero_kicker) || 'MINIMAL • MODERN';
  ...
  const featuredSubtitle = asString(settings.template_featured_subtitle) || 'A neat selection, well presented.';
  ...
  return (
    <div className="ecopro-storefront" data-edit-path="__root" style={{ minHeight: '100vh', background: bg, color: text, fontFamily }}>
      <header data-edit-path="layout.header" onClick={(e) => clickGuard(e, 'layout.header')} style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(250,250,250,0.9)', backdropFilter: 'blur(8px)', borderBottom: '1px solid rgba(15,23,42,0.10)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: `${spacing}px ${spacing}px`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14 }}>
          <div data-edit-path="layout.header.logo" onClick={(e) => clickGuard(e, 'layout.header.logo')} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: canManage ? 'pointer' : 'default' }}>
            {logoUrl && <img src={logoUrl} alt={storeName} style={{ width: 38, height: 38, borderRadius: 12, objectFit: 'cover', border: '1px solid rgba(15,23,42,0.12)' }} />}
      ...
      <section data-edit-path="layout.hero" onClick={(e) => clickGuard(e, 'layout.hero')} style={{ padding: `${sectionSpacing}px ${spacing}px`, position: 'relative' }}>
        <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(135deg, rgba(14,165,233,0.10), transparent 45%), linear-gradient(225deg, rgba(99,102,241,0.08), transparent 45%)` }} />
              ...
              {products.map((p) => (
                <div key={p.id} data-edit-path={`layout.products.${p.id}`} onClick={(e) => { if (canManage) return clickGuard(e, `layout.products.${p.id}`); if ((p as any).slug) navigate((p as any).slug); }} style={{ background: cardBg, borderRadius: cardRadius, overflow: 'hidden', border: '1px solid rgba(15,23,42,0.10)', cursor: 'pointer', boxShadow: '0 1px 0 rgba(15,23,42,0.06)' }}>
                  ...
                  <div style={{ padding: 12, display: 'grid', gap: 8 }}>
                    ...
                    <button data-edit-path="layout.featured.addLabel" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.featured.addLabel'); e.stopPropagation(); if ((p as any).slug) navigate((p as any).slug); }} style={{ justifySelf: 'start', background: 'rgba(14,165,233,0.12)', color: text, border: '1px solid rgba(14,165,233,0.22)', borderRadius: borderRadius, padding: '8px 10px', fontSize: 12, fontWeight: 900, cursor: 'pointer' }}>{addLabel}</button>
                  </div>
                </div>
              ))}
            </div>
