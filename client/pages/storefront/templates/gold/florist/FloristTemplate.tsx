...
export default function FloristTemplate(props: TemplateProps) {
  ...
  const accent = settings.template_accent_color || '#e11d48';
  const cardBg = settings.template_card_bg || '#ffffff';

  const storeName = settings.store_name || 'Bloom & Blossom';
...
const heroTitle = settings.template_hero_heading || 'Fresh Flowers 💐';
  const heroSubtitle = settings.template_hero_subtitle || 'Beautiful bouquets for every occasion';
  const ctaText = settings.template_button_text || 'Order Now';
...
const products = useMemo(() => {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: bg, color: text, fontFamily: 'Georgia, serif' }} data-edit-path="__root">
      ...
      <section style={{ padding: '80px 24px', textAlign: 'center', background: `radial-gradient(circle at center, #fce7f3 0%, ${bg} 70%)` }} data-edit-path="layout.hero" onClick={(e) => clickGuard(e, 'layout.hero')}>
      ...
      <section style={{ padding: `${sectionSpacing}px 24px`, maxWidth: '1400px', margin: '0 auto' }} data-edit-path="layout.grid">
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: `${gridGap}px` }}>
          {products.map(product => (
            <div key={product.id} style={{ backgroundColor: cardBg, borderRadius: cardRadius, overflow: 'hidden', boxShadow: '0 4px 20px rgba(225,29,72,0.1)', cursor: canManage ? 'default' : 'pointer', transition: `transform ${animationSpeed}ms, box-shadow ${animationSpeed}ms` }} data-edit-path={`layout.grid.items.${product.id}`}
              ...
              <div style={{ padding: '20px', textAlign: 'center' }}>
                <p style={{ fontSize: '10px', letterSpacing: '2px', color: productPriceColor, marginBottom: '8px' }}>{product.category || 'FLOWERS'}</p>
      ...
      <footer style={{ padding: '48px 24px', backgroundColor: '#fce7f3', marginTop: '48px', textAlign: 'center' }} data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')}>
        <p style={{ color: '#9f1239', fontStyle: 'italic' }}>{copyright}{storeDescription ? ` — ${storeDescription}` : ''} 🌸</p>
      </footer>
    </div>
  );
}
