...
export default function OrbitMartTemplate(props: TemplateProps) {
  ...
  const storeName = asString(settings.store_name) || 'ORBIT MART';
  ...
  const bg = asString(settings.template_bg_color) || '#050816';
  const text = asString(settings.template_text_color) || '#e2e8f0';
  ...
  const heroTitle = asString(settings.template_hero_heading) || 'Orbit-fast shopping experience.';
  const heroSubtitle = asString(settings.template_hero_subtitle) || 'Neat layout with subtle sci-fi glow.';
  const heroKicker = asString(settings.template_hero_kicker) || 'ORBIT • MARKET';
  ...
  const featuredSubtitle = asString(settings.template_featured_subtitle) || 'New items in your orbit.';
  ...
  return (
    <div className="ecopro-storefront" data-edit-path="__root" style={{ minHeight: '100vh', background: bg, color: text, fontFamily }}>
      <header data-edit-path="layout.header" onClick={(e) => clickGuard(e, 'layout.header')} style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(5,8,22,0.7)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(34,197,94,0.18)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: `${spacing}px ${spacing}px`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14 }}>
          <div data-edit-path="layout.header.logo" onClick={(e) => clickGuard(e, 'layout.header.logo')} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: canManage ? 'pointer' : 'default' }}>
            {logoUrl && <img src={logoUrl} alt={storeName} style={{ width: 38, height: 38, borderRadius: 9999, objectFit: 'cover', border: '2px solid rgba(34,197,94,0.55)' }} />}
          ...
          <nav data-edit-path="layout.header.nav" onClick={(e) => clickGuard(e, 'layout.header.nav')} style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            {navLinks.map((l) => (
              <button key={`${l.label}-${l.url}`} onClick={(e) => { if (canManage) return clickGuard(e, 'layout.header.nav'); navigate(l.url || '/products'); }} style={{ background: 'rgba(226,232,240,0.06)', border: '1px solid rgba(226,232,240,0.12)', color: text, borderRadius: borderRadius, padding: '7px 10px', fontSize: 12, fontWeight: 900, cursor: 'pointer' }}>
                {l.label || 'Link'}
              </button>
            ))}
          </nav>
      ...
      <section data-edit-path="layout.hero" onClick={(e) => clickGuard(e, 'layout.hero')} style={{ padding: `${sectionSpacing}px ${spacing}px`, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(900px 420px at 20% 25%, rgba(34,197,94,0.18), transparent 60%), radial-gradient(900px 420px at 80% 20%, rgba(59,130,246,0.12), transparent 60%)' }} />
        {bannerUrl && (
        ...
        <div style={{ position: 'relative', maxWidth: 1200, margin: '0 auto', display: 'grid', gap: 16 }}>
          ...
          <p data-edit-path="layout.hero.subtitle" onClick={(e) => clickGuard(e, 'layout.hero.subtitle')} style={{ margin: 0, maxWidth: 720, color: 'rgba(226,232,240,0.78)', fontSize: 15, lineHeight: 1.8 }}>{heroSubtitle}</p>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 6 }}>
            <button data-edit-path="layout.hero.cta" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.hero.cta'); navigate('/products'); }} style={{ background: accent, color: '#05120a', border: 0, borderRadius: buttonRadius, padding: '12px 18px', fontWeight: 950, cursor: 'pointer' }}>{ctaText}</button>
            <button data-edit-path="layout.hero.cta2" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.hero.cta2'); navigate('/products'); }} style={{ background: 'transparent', color: text, border: '1px solid rgba(226,232,240,0.20)', borderRadius: buttonRadius, padding: '12px 18px', fontWeight: 950, cursor: 'pointer' }}>{asString(settings.template_button2_text) || 'Explore'}</button>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div data-edit-path="layout.products">
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${gridColumns}, minmax(0, 1fr))`, gap: gridGap }}>
              {products.map((p) => (
                <div key={p.id} data-edit-path={`layout.products.${p.id}`} onClick={(e) => { if (canManage) return clickGuard(e, `layout.products.${p.id}`); if ((p as any).slug) navigate((p as any).slug); }} style={{ background: cardBg, borderRadius: cardRadius, overflow: 'hidden', border: '1px solid rgba(226,232,240,0.14)', cursor: 'pointer' }}>
                  ...
                  <div style={{ padding: 12, display: 'grid', gap: 8 }}>
                    ...
                    <button data-edit-path="layout.featured.addLabel" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.featured.addLabel'); e.stopPropagation(); if ((p as any).slug) navigate((p as any).slug); }} style={{ justifySelf: 'start', background: 'rgba(34,197,94,0.16)', color: text, border: '1px solid rgba(34,197,94,0.24)', borderRadius: borderRadius, padding: '8px 10px', fontSize: 12, fontWeight: 900, cursor: 'pointer' }}>{addLabel}</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')} style={{ padding: `${spacing}px ${spacing}px`, borderTop: '1px solid rgba(226,232,240,0.16)', background: 'rgba(5,8,22,0.7)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div data-edit-path="layout.footer.social" onClick={(e) => clickGuard(e, 'layout.footer.social')} style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {(socialLinks.length ? socialLinks : canManage ? [{ platform: 'instagram', url: 'https://instagram.com' }] : []).map((s) => (
              <a key={`${s.platform}-${s.url}`} href={canManage ? undefined : s.url} onClick={(e) => { if (canManage) return clickGuard(e as any, 'layout.footer.social'); e.stopPropagation(); }} style={{ color: text, textDecoration: 'none', fontSize: 12, padding: '6px 10px', borderRadius: borderRadius, border: '1px solid rgba(226,232,240,0.18)' }}>
