...
export default function RoyalBoutiqueTemplate(props: TemplateProps) {
  ...
  const storeName = asString(settings.store_name) || 'ROYAL BOUTIQUE';
  ...
  const sectionSpacing = resolveInt(settings.template_section_spacing, 72, 24, 130);

  const heroTitle = asString(settings.template_hero_heading) || 'Luxury, polished.';
  const heroSubtitle = asString(settings.template_hero_subtitle) || 'Elegant serif hero with gold accents.';
  const heroKicker = asString(settings.template_hero_kicker) || 'ROYAL • EDIT';
  ...
  const featuredSubtitle = asString(settings.template_featured_subtitle) || 'A refined selection.';
  ...
  return (
    <div className="ecopro-storefront" data-edit-path="__root" style={{ minHeight: '100vh', background: bg, color: text, fontFamily }}>
      <header data-edit-path="layout.header" onClick={(e) => clickGuard(e, 'layout.header')} style={{ position: 'sticky', top: 0, zIndex: 20, background: 'rgba(10,7,20,0.7)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(251,191,36,0.20)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: `${spacing}px ${spacing}px`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14 }}>
          <div data-edit-path="layout.header.logo" onClick={(e) => clickGuard(e, 'layout.header.logo')} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: canManage ? 'pointer' : 'default' }}>
            {logoUrl && <img src={logoUrl} alt={storeName} style={{ width: 38, height: 38, borderRadius: 14, objectFit: 'cover', border: '1px solid rgba(251,191,36,0.35)' }} />}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <div style={{ fontWeight: 950, letterSpacing: '0.10em', textTransform: 'uppercase' }}>{storeName}</div>
          ...
          <nav data-edit-path="layout.header.nav" onClick={(e) => clickGuard(e, 'layout.header.nav')} style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            {navLinks.map((l) => (
              <button key={`${l.label}-${l.url}`} onClick={(e) => { if (canManage) return clickGuard(e, 'layout.header.nav'); navigate(l.url || '/products'); }} style={{ background: 'rgba(251,191,36,0.10)', border: '1px solid rgba(251,191,36,0.18)', color: text, borderRadius: borderRadius, padding: '7px 10px', fontSize: 12, fontWeight: 900, cursor: 'pointer' }}>
                {l.label || 'Link'}
              </button>
            ))}
          </nav>
      ...
      <section data-edit-path="layout.hero" onClick={(e) => clickGuard(e, 'layout.hero')} style={{ padding: `${sectionSpacing}px ${spacing}px`, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(900px 420px at 20% 25%, rgba(251,191,36,0.18), transparent 60%), radial-gradient(900px 420px at 80% 20%, rgba(139,92,246,0.10), transparent 60%)' }} />
        {bannerUrl && (
          <div data-edit-path="layout.hero.image" onClick={(e) => clickGuard(e, 'layout.hero.image')} style={{ position: 'absolute', inset: 0, opacity: 0.14 }}>
        ...
        <div style={{ position: 'relative', maxWidth: 1200, margin: '0 auto', display: 'grid', gap: 16 }}>
          ...
          <h1 data-edit-path="layout.hero.title" onClick={(e) => clickGuard(e, 'layout.hero.title')} style={{ margin: 0, fontSize: 58, lineHeight: 1.02, fontWeight: 900 }}>{heroTitle}</h1>
          <p data-edit-path="layout.hero.subtitle" onClick={(e) => clickGuard(e, 'layout.hero.subtitle')} style={{ margin: 0, maxWidth: 760, color: 'rgba(248,250,252,0.78)', fontSize: 16, lineHeight: 1.85 }}>{heroSubtitle}</p>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 6 }}>
            <button data-edit-path="layout.hero.cta" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.hero.cta'); navigate('/products'); }} style={{ background: accent, color: '#0a0714', border: 0, borderRadius: buttonRadius, padding: '12px 18px', fontWeight: 950, cursor: 'pointer' }}>{ctaText}</button>
            <button data-edit-path="layout.hero.cta2" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.hero.cta2'); navigate('/products'); }} style={{ background: 'transparent', color: text, border: '1px solid rgba(248,250,252,0.20)', borderRadius: buttonRadius, padding: '12px 18px', fontWeight: 950, cursor: 'pointer' }}>{asString(settings.template_button2_text) || 'Browse'}</button>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div data-edit-path="layout.products">
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${gridColumns}, minmax(0, 1fr))`, gap: gridGap }}>
              {products.map((p) => (
                <div key={p.id} data-edit-path={`layout.products.${p.id}`} onClick={(e) => { if (canManage) return clickGuard(e, `layout.products.${p.id}`); if ((p as any).slug) navigate((p as any).slug); }} style={{ background: cardBg, borderRadius: cardRadius, overflow: 'hidden', border: '1px solid rgba(251,191,36,0.18)', cursor: 'pointer' }}>
                  ...
                  <div style={{ padding: 12, display: 'grid', gap: 8 }}>
                    ...
                    <button data-edit-path="layout.featured.addLabel" onClick={(e) => { if (canManage) return clickGuard(e, 'layout.featured.addLabel'); e.stopPropagation(); if ((p as any).slug) navigate((p as any).slug); }} style={{ justifySelf: 'start', background: 'rgba(251,191,36,0.14)', color: text, border: '1px solid rgba(251,191,36,0.22)', borderRadius: borderRadius, padding: '8px 10px', fontSize: 12, fontWeight: 900, cursor: 'pointer' }}>{addLabel}</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer data-edit-path="layout.footer" onClick={(e) => clickGuard(e, 'layout.footer')} style={{ padding: `${spacing}px ${spacing}px`, borderTop: '1px solid rgba(251,191,36,0.20)', background: 'rgba(10,7,20,0.65)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div data-edit-path="layout.footer.social" onClick={(e) => clickGuard(e, 'layout.footer.social')} style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {(socialLinks.length ? socialLinks : canManage ? [{ platform: 'instagram', url: 'https://instagram.com' }] : []).map((s) => (
              <a key={`${s.platform}-${s.url}`} href={canManage ? undefined : s.url} onClick={(e) => { if (canManage) return clickGuard(e as any, 'layout.footer.social'); e.stopPropagation(); }} style={{ color: text, textDecoration: 'none', fontSize: 12, padding: '6px 10px', borderRadius: borderRadius, border: '1px solid rgba(248,250,252,0.18)' }}>
