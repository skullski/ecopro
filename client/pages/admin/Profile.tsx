...
export default function Profile() {
  ...
  return (
    <>
      ...
      <div className="space-y-2 w-full min-h-screen p-3 -m-3 relative z-10">
        ...
        <div className="flex items-center justify-between backdrop-blur-md profile-glass rounded-lg p-2 border border-white/30 dark:border-slate-700/50">
          <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
          {/* Subscription (gold) - FIRST */}
          <div
            className={
              isLight
                ? "rounded-lg overflow-hidden border border-amber-200/70 shadow-md bg-white/55 backdrop-blur-md"
                : "rounded-lg overflow-hidden border border-slate-700/50 shadow-md bg-slate-900/95 backdrop-blur-md"
            }
          >
            <div
              className={
                isLight
                  ? "bg-gradient-to-r from-amber-200/70 to-orange-200/60 p-2.5 text-center border-b border-amber-300/40"
                  : "bg-gradient-to-r from-amber-500/30 to-orange-500/30 p-2.5 text-center border-b border-amber-500/30"
              }
            >
              <div
                className={
                  isLight
                    ? "w-8 h-8 bg-amber-300/40 rounded-full flex items-center justify-center mx-auto mb-1.5 border border-amber-400/40"
                    : "w-8 h-8 bg-amber-500/20 rounded-full flex items-center justify-center mx-auto mb-1.5 border border-amber-500/50"
                }
              >
                <Lock className={isLight ? "w-4 h-4 text-amber-700" : "w-4 h-4 text-amber-400"} />
              </div>
            ...
            <div className="p-2 space-y-1.5">
              ...
              <div className="bg-slate-800/60 rounded-lg p-2 border border-slate-700/50 space-y-2">
                ...
                {voucherSuccess ? (
                  ...
                  <form onSubmit={handleRedeemVoucher} className="space-y-1.5">
                    <Input
                      ...
                      className={
                        isLight
                          ? "h-7 bg-amber-200/80 border-amber-300 text-slate-900 placeholder:text-amber-800/70 font-mono text-center text-xs"
                          : "h-7 bg-slate-900/50 border-slate-600 text-white placeholder:text-slate-500 font-mono text-center text-xs"
                      }
                    />
                    ...
                    <Button
                      ...
                      className="w-full bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 text-white h-7 font-semibold text-xs"
                      size="sm"
                    >
                      {voucherLoading ? (
                        <>
          ...
          <CardContent className="space-y-2 p-2 pt-0">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
              <div className="space-y-0.5">
                ...
                <Input id="name" className="h-7" value={form.name} onChange={(e) => setForm((s) => ({ ...s, name: e.target.value }))} />
              </div>
              <div className="space-y-0.5">
                <Label htmlFor="email" className="text-xs">{t('admin.profile.email')}</Label>
                <Input id="email" className="h-7" value={form.email} onChange={(e) => setForm((s) => ({ ...s, email: e.target.value }))} />
              </div>
              <div className="space-y-0.5">
                <Label htmlFor="phone" className="text-xs">{t('admin.profile.phone')}</Label>
                <Input id="phone" className="h-7" value={form.phone} onChange={(e) => setForm((s) => ({ ...s, phone: e.target.value }))} />
              </div>
              <div className="space-y-0.5">
              ...
              <div className="space-y-0.5">
                ...
                <Input id="country" className="h-7" value={form.country} onChange={(e) => setForm((s) => ({ ...s, country: e.target.value }))} />
              </div>
              <div className="space-y-0.5">
                <Label htmlFor="city" className="text-xs">{t('admin.profile.city')}</Label>
                <Input id="city" className="h-7" value={form.city} onChange={(e) => setForm((s) => ({ ...s, city: e.target.value }))} />
              </div>
            </div>

            <Button onClick={onSave} disabled={saving || loading} className="w-full h-7" size="sm">
        ...
        <Card className="p-0 backdrop-blur-md profile-glass border-white/30 dark:border-slate-700/50">
          ...
          <CardContent className="space-y-2 p-2 pt-0">
            ...
            <form onSubmit={handleChangePassword} className="space-y-1.5">
              <div className="space-y-0.5">
              ...
              <Button type="submit" disabled={passwordLoading} className="w-full h-7" size="sm">
                {passwordLoading ? (
                  <>
                    <Loader className="w-3 h-3 mr-1 animate-spin" />
